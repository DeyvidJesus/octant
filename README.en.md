...
- **JSONB integrity in two layers**: CHECK constraints on every JSONB column (document id matches the row,
  required fields present, size capped; added `NOT VALID` so legacy rows never block a deploy), and zod
  schemas on read that repair safe defaults and drop, with a report, what cannot be trusted.
...
- **Stripe webhook**: signature-verified; the subscription upsert is the contract (DB error → 500 so
  Stripe retries) and applies only events newer than the last one, atomically in SQL. Billing emails
  are best-effort, and plan changes reach the app over realtime.
...
| **Quality** | Vitest + Testing Library (happy-dom), PGlite for database tests, zod, oxlint, GitHub Actions |
...
├── migrations/    # Schema history (0001 → 0018)
└── tests/         # Database tests on PGlite (schema, migrations, RLS, constraints)
```
...
Database: run `supabase-schema.sql`, then every file in `supabase/migrations` in order, in the Supabase SQL
editor. Edge
...
| `yarn test` | Vitest (≈510 tests, database tests included) |
...
and toasts. Database tests run the schema and every migration on PGlite (Postgres compiled to
WebAssembly) and check the JSONB constraints, RLS isolation, plan caps and Stripe event ordering. CI runs lint, tests, the build, `deno check` on the
