 ...
 * into JSON; all validation, normalization, and truth decisions happen in
 * deterministic code below. A malformed or inventive model output can degrade
 * into dropped entries and warnings — never into fabricated board data.
 */
...
/**
 * Hand-rolled validation of the parsed array (same style as backup.ts):
 * entries without a company AND role are dropped with a warning — code decides
 * what survives, not the model.
 */
