...
import { parseJobsJson, normalizeCandidates, toCandidates } from '@/services/ai/tasks/extractJobs'
...
// deno-lint-ignore no-explicit-any
async function freeTierOverBudget(admin: any, userId: string, tier: PlanTier): Promise<boolean> {
  if (tier === 'pro') return false
  const limit = Number(Deno.env.get('FREE_TIER_MONTHLY_TOKEN_LIMIT') ?? '100000')
...
async function runForUser(admin: any, apiKey: string, userId: string, trigger: string): Promise<void> {
  ...
  if (await freeTierOverBudget(admin, userId, tier)) return
